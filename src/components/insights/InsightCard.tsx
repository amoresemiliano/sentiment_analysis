...
export const InsightCard: React.FC<InsightCardProps> = ({ insight, onDrillDown }) => {
  ...
  return (
    <article className="bg-white border border-stone-200/90 rounded-2xl p-5 sm:p-6 shadow-xs space-y-4 hover:border-stone-400 transition-all font-['Plus_Jakarta_Sans']">
      ...
      {/* Observation Block (WHAT WE FOUND) */}
      ...
      {/* Interpretation Block (WHY IT MATTERS / PROBABILISTIC INTERPRETATION) */}
      <div className="p-3.5 bg-stone-50 border border-stone-200/70 rounded-xl space-y-1">
        <div className="flex items-center gap-1.5 text-stone-700 text-xs font-bold uppercase tracking-wider">
          <Sparkles className="w-3.5 h-3.5 text-[#1B4D3E]" />
          <span>Interpretación Analítica (Qué podría significar)</span>
        </div>
      ...
      <InsightBusinessQuestion question={insight.businessQuestion} />

      {/* Methodology Note if present */}
      {insight.methodologyNote && (
        <div className="text-[10px] text-stone-500 flex items-start gap-1 pt-1">
          <Info className="w-3 h-3 text-stone-400 shrink-0 mt-0.5" />
          <span>{insight.methodologyNote}</span>
        </div>
      )}
      ...
      <div className="pt-2 border-t border-stone-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="text-xs text-stone-500">
          Evidencia trazable: <strong className="text-stone-800">{insight.evidence.reviewIds.length} verbatims</strong> vinculados a este insight
        </div>
        ...
        >
          ...
          <span>Ver Evidencia ({insight.evidence.reviewIds.length} reviews)</span>
